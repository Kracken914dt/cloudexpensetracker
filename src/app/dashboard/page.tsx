    ...
    const fetchData = async () => {
      try {
        ...
        const [dashboardData, expensesData] = await Promise.all([
          ...
          ExpenseService.getAll({ from: new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().split('T')[0] })
        ]);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
             ...
             <Card className="h-full p-6">
               ...
               <div className="flex items-end justify-between h-48 gap-4">
                 {Array.from({ length: 6 }).map((_, i) => (
                   <Skeleton key={i} className="w-full h-full rounded-t-md opacity-50" style={{ height: `${[40, 70, 30, 80, 50, 60][i]}%` }} />
                 ))}
               </div>
             </Card>
          ) : (
        ...
        <Card className="col-span-3">
          <div className="p-6">
            <h3 className="text-lg font-semibold mb-4 text-foreground">Transacciones Recientes</h3>
